...
let ps s l =
  ...
  | [] -> [ s ]
  | h :: t ->
      ((s ^ "(") :: List.fold_left (fun l x -> comma l @ indent x) (indent h) t)
      @ [ ")" ]
...
and pe e =
  ...
  | LETF (f, xs, e1, e2) -> ps "LETF" [ q f; ps "ARGS" [ qs xs ]; pe e1; pe e2 ]
  | CALLV (f, es) -> ps "CALLV" [ q f; ps "ARGS" (List.map pe es) ]
  | CALLR (f, ys) -> ps "CALLR" [ q f; ps "ARGS" (List.map pe ys) ]
